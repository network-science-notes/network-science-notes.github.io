...
plot_kwargs = {"node_size" : 50, "edgecolors" : 'black', "node_color" : "steelblue", "width" : 0.5}
...
One way to compute the degrees of the nodes in a graph is to use the adjacency matrix: 

$$
    k_i = \sum_{j=1}^n A_{ij} \,.
$$

Using `numpy`, we can do this efficiently for every node at once: 
...
Suppose that you know the in-degree and out-degree of every node in a graph $G$. Determine: 

1. The total number of edges in the graph.
2. The mean in-degree and out-degree of the graph.
...
Let's see these definitions in action on a directed graph. The graph we'll use is the *mention graph* from the musical *Hamilton*. Each node in this graph is a character. There is a directed edge from character $i$ to character $j$ if character $i$ mentions character $j$ in a song.
...
nx.draw(G_Hamilton, pos = layout,  with_labels = True, edge_color = "grey", width = 0.5, node_color = "lavender", edgecolors = "white")
...
Similar to the above, let's calculate the in- and out-degrees of the nodes in the Hamilton network using the adjacency matrix and compare to the networkx built-in versions. 
...
Repeat this calculation and check that you can also compute *out*-degrees and make them match. 
...
nx.draw(G, with_labels = True, font_color = 'white', font_weight = 'bold', edgecolors = "black", ax = ax, node_size = 200, font_size = 10)
```
...
A few important points about paths: 
...
Determine one criterion for a graph to be connected by considering the matrix 

$$
\mathbf{M} = \sum_{i = 1}^n \mathbf{A}^i \,.
$$
...
Give an algorithm for determining whether a graph is connected using a search method of your choosing.  
...
Now that we're convinced that the algorithm works, let's go ahead and implement it in Python. 
...
Now we can run a few examples: 

```{python}
#| fig-cap : "The binomial tree example"
#| out.width : 80%
#| fig-cap-location: margin
...
nx.draw(G_Tree,  font_weight = 'bold', edgecolors = "black", node_size = 50, font_size = 10, node_color = "lavender", edge_color = "grey")
is_cyclic(G_Tree)
```


```{python}
#| fig-cap : "The Hamilton mention network"
#| out.width : 80%
#| fig-cap-location: margin
#| column: margin
...
fig, ax = plt.subplots(1, 1, figsize = (3, 3))
nx.draw(G_Hamilton, pos = layout, font_weight = 'bold', edgecolors = "black", node_size = 50, font_size = 10, node_color = "lavender", edge_color = "grey")
...
A **tree** is a connected, acyclic, undirected network. By "connected", we mean every node is reachable from every other node by traversing a sequence of edges, i.e., there exists a walk between any two nodes
