...
#| fig-cap: "A $G(n,p)$ random graph with $n = 100$ and $p = 0.03$."
#| code-fold: true
nx.draw(G, 
        node_size = 100, 
        edge_color = "gray", 
...
```{python}
#| fig-cap : "Each point gives the global clustering coefficient of an ER graph with mean degree $c = 5$ and specified number of nodes. We've repeated the calculation for each size of graph 10 times. The black line gives the estimate $T = p = c/(n-1)$ for the clustering coefficient that we computed earlier."
