...
1. **Objective**: $\mathrm{NormCut}(C_0, C_1) = 2m\vy^T \mL \vy$, where $\mD$ is the diagonal matrix with entries $d_{ii} = k_i$, the degree of node $i$, and $\mL = \mD - \mA$ is our good friend the combinatorial graph Laplacian. 
2. **Normalization**: $\vy^T\mD\vy = 1$. 
2. **Orthogonality**: $\vy^T\mD \vones = 0$, where $\vones$ is the all-ones vector.
...
This clustering looks pretty reasonable! 
